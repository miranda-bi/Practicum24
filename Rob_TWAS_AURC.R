...
TWAS_Rob_v6 <- parallel::mclapply(1:22, function(chr){
  # chr=1
  ...
  chrres <- suppressWarnings(as.data.frame(rbindlist( lapply(genenames, function(geneid){
    ...
    if( var(mod[,geneid])>0 ){
      fit <- tryCatch(coeftest(lm(form, data=dat.twas.chr), vcov = sandwich), error=function(e){print(e); NULL}) # use robust var
    }else fit <- NULL
    ...
    if( is.null(fit) ){
      ...
      if( sum(idrows)==2 ){
        p2df <- tryCatch(wald.test(b=fit[,1], Sigma=fit[,2], Terms=which(idrows))$result$chi2, error = function(e){print(e); NA})
...
TWAS_Rob_v6 <- as.data.frame(rbindlist(TWAS_Rob_v6))

# Save the TWAS results and summaries into an RData file in the specified output directory
save(TWAS_Rob_v6, file=paste0(OUTDIR,"Rob_TWAS_AURC_v6_PC1_",folder,".RData"))
